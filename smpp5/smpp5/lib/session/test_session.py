...
class SMPPSession(object):
    ...
    def storing_recieved_pdus(self):
        ...
        if(R):
            if(R.sequence_number.value in pdus[R.sequence_number.value]):
                pdus[R.sequence_number.value]['resp'] = R
                pdus[R.sequence_number.value]['read'] = 'false'

    def processing_recieved_pdus(self):
        for seq_no in pdus:
            if pdus[seq_no]['read'] == 'false':
                R = pdus[seq_no]['resp']
    ...
    def bind(self, bind_type, system_id, password, system_type):
        ...
        print("bind mai tou aa raha hai")
        self.socket.send(data)
        # recieving the response from server
        #P = self.get_pdu_from_socket()

    def binding_response_handling(self, R):
        transmitter_id = command_ids.bind_transmitter_resp
        receiver_id = command_ids.bind_receiver_resp
        transceiver_id = command_ids.bind_transceiver_resp
        bind_types = dict(
            transmitter_id=dict(state=SessionState.BOUND_TX),
            receiver_id=dict(state=SessionState.BOUND_RX),
            transceiver_id=dict(state=SessionState.BOUND_TRX)
        )
        if(R.command_status.value == 0):
            self.state = bind_types[R.command_id.value]['state']
